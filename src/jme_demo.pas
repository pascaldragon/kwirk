...
Procedure WriteDienst(x,y,tc,bc: integer);
  ...
  GotoXY(x,y+06); write('                                    ');
  GotoXY(x,y+07); write(' - kaufmÑnnische Software           ');
  GotoXY(x,y+08); write('   Finanzbuchhaltung                ');
  GotoXY(x,y+09); write('   KassenbuchfÅhrung                ');
  GotoXY(x,y+10); write('   Adre·verwaltung                  ');
...
Procedure WriteKnowHow(x,y,tc,bc: integer);
  ...
  GotoXY(x,y+02); write(' FÑllen an uns.       ');
  GotoXY(x,y+03); write(' Wo andere aufhîren,  ');
...
Procedure WriteJMELogo(x,y,tc,bc: integer);
  begin
  SetColor(tc,bc);
  GotoXY(x,y+00); write('  ƒƒƒƒƒƒƒ¬ƒƒƒƒ¬ƒƒƒƒ¬ƒƒƒƒƒƒƒ  ');
  GotoXY(x,y+01); write('  ƒƒƒƒƒƒƒŸ         ¿ƒƒƒƒƒƒƒ  ');
  end;
...
Procedure WriteNote(x,y,tc,bc: integer);
  ...
  GotoXY(x,y+03); write(' LauffÑhig mit CGA-, Hercules-, EGA- und VGA-Grafikkarten. ');
  end;
...
Procedure WritePressKey(x,y,tc1,bc1,tc2,bc2: integer);
  ...
  GotoXY(x,y);
  SetColor(tc1,bc1);
  write(' DrÅcken Sie eine Taste, um ');
  SetColor(tc2,bc2);
